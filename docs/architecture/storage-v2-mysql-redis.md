...
这份规范是 Bloeco 经济中心及后续接入插件的共同存储契约。Bloeco 负责全服货币账本、余额、发行/回收、税费、手续费、国有银行、幂等和审计；业务插件只拥有自己的业务数据，并通过 Bloeco API 提交经济结算。
...
当前正式表为 `banks`、`bank_deposits`、`bank_loans`、`bank_loan_payments` 和 `bank_operations`。银行现金仍是中央货币总账中的 `bank:cash` 账户；存款和贷款表只记录银行负债、应收和合同事实，不能被重复计入基础货币供给。每笔存取款、放款和还款必须关联永久 `journal_id` 与稳定幂等操作记录。

贷款利率按整数年化基点保存，并在放款时快照到贷款合同。默认值为 `320` 基点、期限 7 天；利息使用整数最小货币单位按 `本金 × 年化基点 × 天数 ÷ 3,650,000` 计算。政策修改只影响新贷款，不修改既有贷款。
