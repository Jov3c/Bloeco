...
运行时所有账本和银行写入均通过有界异步队列进入存储层；Paper 主线程不执行 JDBC 或 Redis 网络操作。队列关闭或饱和时操作安全失败，不改变余额。MySQL 是权威来源，Redis 只负责可重建缓存和 outbox 事件加速，故障时保留未发布事件并继续以数据库结果为准。
...
计划中的第三方插件接口通过 Paper `ServicesManager` 注册并提供 `BloecoApi`。Bloeco 将使用调用方的实际 `Plugin` 实例识别身份，并生成规范化 `client_id`；插件不能在请求参数中冒充其他插件。该 API 尚未作为兼容工件发布，当前接入方不得直接调用内部实现或数据库。
...
Bloeco 内置单一国有银行 `bloeco.bank`。首次建立时，配置的初始资本由国库真实划拨到 `bank:cash`，只执行一次，不调用发行流程。玩家存款是钱包到银行现金的转移，取款执行反向转移；贷款只能从满足准备金约束后的银行现有现金中发放，因此存取款、贷款和还款都不改变净货币供给。

默认贷款采用 3.2% 年化固定利率、7 天期限。利息以整数最小货币单位按 `本金 × 年化基点 × 期限天数 ÷ 3,650,000` 计算，正数贷款最低利息为 `0.01`。放款时把合同利率和到期时间写入贷款记录，管理员后续修改政策不得追溯改变已有贷款。

玩家从 `/eco -> 国有银行` 使用账户详情、存款、取款、贷款和还款；信用等级与 A–D 标准只在账户详情显示。转账、存款、取款和贷款提供 100/1000/10000 预设与聊天栏自定义金额，存取款另提供按事务实时余额执行的“全部存入”和“全部取出”；聊天输入支持取消和 60 秒超时。普通玩家提示只显示业务结果、金额和税费，不展示中央总账或内部清算术语。银行政策由 `bloeco.role.banker` 管理，该角色不包含发行或税收权限。

## 6. 计划中的原生异步 API

后续独立 API 版本将通过 Paper `ServicesManager` 暴露稳定的 Java 接口，不依赖 Vault。API 实现返回 `CompletionStage<Result<T>>`，数据库完成后才结束；回调线程不保证为 Paper 主线程。当前 1.3 源码只提供 Bloeco 内部异步边界，第三方兼容 API 尚未发布。
...
- `bloeco.role.banker`：查看银行资产负债表并管理存款利率、贷款年化利率、准备金率、贷款上限和放贷开关。
...
3. 全新账本的新玩家默认从国库领取一次 `100.00` 启动资金；该金额可配置、按 UUID 永久去重，且除发行外没有任何路径可增加净货币供给。
...
11. 国有银行资本、存取款、贷款和还款全部使用既有货币；默认贷款年化 3.2%、期限 7 天，政策变更不追溯既有合同。
