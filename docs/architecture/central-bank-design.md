...
玩家从 `/eco -> 国有银行` 使用账户详情、存款、取款、贷款、还款和信用等级。转账、存款、取款和贷款提供 100/1000/10000 预设与聊天栏自定义金额；聊天输入支持取消和 60 秒超时。普通玩家提示只显示业务结果、金额和税费，不展示中央总账或内部清算术语。银行政策由 `bloeco.role.banker` 管理，该角色不包含发行或税收权限。
